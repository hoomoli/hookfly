  ...
  return (
    <>
      ...
      <section className="py-5" aria-labelledby="targets-summary-title">
        ...
        {targets.length > 0 && <section className="overflow-hidden rounded-2xl border border-border bg-card shadow-[0_1px_2px_rgb(0_0_0/0.04)]"><Table className="min-w-[1080px]"><TableHeader className="bg-muted/35"><TableRow className="hover:bg-transparent"><TableHead>{t("targets.targetID")}</TableHead><TableHead>{t("targets.connection")}</TableHead><TableHead>{t("targets.projectEnvironment")}</TableHead><TableHead>{t("targets.compose")}</TableHead><TableHead>{t("targets.appName")}</TableHead><TableHead>{t("targets.composeID")}</TableHead><TableHead>{t("targets.composeStatus")}</TableHead><TableHead>{t("targets.runtimeStatus")}</TableHead></TableRow></TableHeader><TableBody className="data-surface">{targets.map((target) => {
          ...
          return <TableRow key={target.id}><TableCell><strong className="font-semibold text-primary">{target.id}</strong></TableCell><TableCell><span className="block">{target.connection_id}</span><small className="text-muted-foreground">{target.resource_type}</small></TableCell><TableCell><span className="block">{target.project_name || "—"}</span><small className="text-muted-foreground">{target.environment_name || "—"}</small></TableCell><TableCell>{target.name || "—"}</TableCell><TableCell>{target.app_name || "—"}</TableCell><TableCell><code>{target.resource_id}</code></TableCell><TableCell><Badge tone={deploymentTone(target.status)}>{t(`targets.deploymentStatuses.${deploymentKey(target.status)}`)}</Badge></TableCell><TableCell><div className="flex min-w-48 items-center gap-2"><Badge tone={runtimeTone(runningContainers, totalContainers)}>{runningContainers} / {totalContainers}</Badge>{target.containers.length > 0 && <details><summary className="cursor-pointer font-medium text-primary">{t("targets.containerDetails")}</summary><ul className="mt-2 space-y-2">{target.containers.map((container) => <li key={container.name} className="rounded-lg border border-border/70 bg-muted/25 p-2"><strong className="block truncate">{container.name}</strong><span className="mt-1 block text-muted-foreground">{t(`targets.containerStates.${container.state}`, { defaultValue: container.state })}{container.health ? ` · ${t("targets.health", { health: t(`targets.healthStatuses.${container.health}`, { defaultValue: container.health }) })}` : ""} · {t("targets.restarts", { count: container.restart_count })}</span></li>)}</ul></details>}</div></TableCell></TableRow>;
