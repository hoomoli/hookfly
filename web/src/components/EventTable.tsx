...
export function EventTable({ repositories, events, selectedID, onSelect, onOperation }: Props) {
  ...
  return <Table className="min-w-[1360px] table-fixed"><TableHeader className="bg-muted/35"><TableRow className="hover:bg-transparent"><TableHead className="w-[170px]">{t("table.received")}</TableHead><TableHead className="w-[180px]">{t("table.repository")}</TableHead><TableHead className="w-[150px]">{t("table.branch")}</TableHead><TableHead className="w-[100px]">{t("table.shortSha")}</TableHead><TableHead className="w-[270px]">{t("table.commit")}</TableHead><TableHead className="w-[260px]">{t("table.event")}</TableHead><TableHead>{t("table.routeTarget")}</TableHead></TableRow></TableHeader><TableBody className="data-surface [&_td]:align-middle">{events.map((event) => {
    ...
    return <TableRow key={event.id} data-state={selected ? "selected" : undefined} className={selected ? "min-h-18 bg-primary/8 hover:bg-primary/12" : "min-h-18"}>
      ...
      <TableCell className="py-3"><code className="text-muted-foreground">{event.revision?.slice(0, 7) ?? "—"}</code></TableCell>
      <TableCell className="py-3"><span className="block truncate text-foreground">{event.commit_message || t("table.noCommitMessage")}</span></TableCell>
