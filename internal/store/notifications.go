...
const notificationFactsUnionSQL = `
    ...
    SELECT '0:push:' || e.id AS id, 'push' AS category, 'received' AS outcome,
           e.id AS event_id, NULL AS delivery_id, NULL AS target_id, e.provider, e.source_id, e.repository_id AS repository,
           'Push received' AS summary, e.received_at AS occurred_at
    FROM events e
    ...
    SELECT '1:delivery:' || d.target_id || ':' || d.id AS id, 'deployment' AS category, 'pending' AS outcome,
           ...
           'Deployment waiting to trigger' AS summary, d.created_at AS occurred_at
    FROM deliveries d
    ...
    FROM (
      SELECT a.id, e.id AS event_id, d.id AS delivery_id, d.target_id, e.provider, e.source_id, e.repository_id AS repository, a.created_at,
             CASE
